...
all.moran <- function(x) {
  result <- moran.test(x, cran.lw) #run moran test on column
...
#test all.moran - IT WORKS
moran.test <- all.moran(cran.sf$GOL_mean)
moran.test

#error message
moran.stats <- purrr::map_dfr(cran.sf[4:37], .f = all.moran)

#error message
moran.stats <- lapply(X=cran.sf[,6], MARGIN = 2, FUN = all.moran)
```