...
all.moranmc <- function(x) {
  result <- moran.mc(x, cran.lw, 10000, na.action = na.omit, zero.policy = TRUE) #run moran test on column
  
  
  df <- data.frame("moran" = as.numeric(result$statistic), 
...
#map function
moranmc.stats <- purrr::map_dfr(cran.df[3:36], .f = all.moranm, .id = "variable")

moranmc.stats
```

```{r evalulate moran.mc}
#filter moran.stats to pval less than 0.05
sig.moranmc <- moranmc.stats %>% filter(., pval <= 0.05)

sig.moranmc
```

```{r subset }
#local moran for WFB_mean
cran.sf$WFB_meanLI <- localmoran(cran.sf$WFB_mean, cran.lw)[, 1]

tm_shape(cran.sf) +
  tm_polygons(col = "WFB_meanLI")
```
