...
#get range of moran I values (from textbook)
moran.range <- function(lw) {
  wmat <- listw2mat(lw)
  return(range(eigen((wmat + t(wmat))/2) $values))
}

print(moran.range(cran.lw))
...
all.moran <- function(x) {
  result <- moran.test(x, cran.lw, na.action = na.omit, zero.policy = TRUE, randomisation = FALSE) #run moran test on column
